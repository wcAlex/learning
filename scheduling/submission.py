...
def create_chain_csp(n):
    ...
    for i in range(len(variables)):
        ...
        if i != 0:
            csp.add_binary_factor(variables[i - 1], variables[i], lambda x1, x2: x1 ^ x2)
...
def create_nqueens_csp(n = 8):
    ...
    variables = ['x%d' % i for i in range(1, n + 1)]
    for i in range(len(variables)):
        csp.add_variable(variables[i], domain[i])

    for i in range(n):
        for j in range(i+1, n):
            csp.add_binary_factor(variables[i], variables[j], lambda p1, p2: abs(p1[0] - p2[0]) != abs(p1[1] - p2[1]))
            csp.add_binary_factor(variables[i], variables[j], lambda p1, p2: p1[1] != p2[1])
...
class BacktrackingSearch():
            ...
            for var in self.csp.variables:
                if var not in assignment:
                    ...
                    for val in self.domains[var]:
                        val_cnt += int(self.get_delta_weight(assignment, var, val))
                    if val_cnt < mcv_cnt:
                        mcv = var
                        mcv_cnt = val_cnt
            return mcv
    ...
    def arc_consistency_check(self, var):
        ...
        def enforceArcConsistency(neighbor, ac_var):
            neighbor_domains = list(self.domains[neighbor])
            ac_var_domains = list(self.domains[ac_var])
            changed = False

            for val_i in neighbor_domains:
                remove = True
                for val_j in ac_var_domains:
                    if self.csp.binaryFactors and self.csp.binaryFactors[ac_var] and self.csp.binaryFactors[ac_var][neighbor] \
                            and self.csp.binaryFactors[ac_var][neighbor][val_j]:
                        if self.csp.binaryFactors[ac_var][neighbor][val_j][val_i] != 0:
                            remove = False

                if remove:
                    changed = True
                    self.domains[neighbor].remove(val_i)
            return changed

        variables = collections.deque([var])
        while len(variables) != 0:
            ac_var = variables.popleft()

            ac_var_domains = list(self.domains[ac_var])
            for val in ac_var_domains:
                if self.csp.unaryFactors[ac_var] != None:
                    if self.csp.unaryFactors[ac_var][val] == 0:
                        self.domains[ac_var].remove(val)
                        variables.append(ac_var)

            for neighbor in self.csp.get_neighbor_vars(ac_var):
                if enforceArcConsistency(neighbor, ac_var):
...
def get_sum_variable(csp, name, variables, maxSum):
    ...
    for idx, var in enumerate(variables):
        ...
        if idx == 0:
            ...
            csp.add_binary_factor(a_i, var, process)
        else:
            csp.add_binary_factor(('sum', name, idx), var, process)
            csp.add_binary_factor(('sum', name, idx - 1), a_i, consistency)

    # consistency
    csp.add_binary_factor(a_last, result, lambda x, y: x[1] == y)
    return result
...
class SchedulingCSPConstructor():
    ...
    def add_unit_constraints(self, csp):
        ...
        unit_scope = (self.profile.minUnits, self.profile.maxUnits)

        for quarter in self.profile.quarters:
            quarter_course_vars = []
            for request in self.profile.requests:
                for cid in request.cids:
                    course_min_unit = self.bulletin.courses[cid].minUnits
                    course_max_unit = self.bulletin.courses[cid].maxUnits
                    course_var = (cid, quarter)
                    domain = range(course_min_unit, course_max_unit + 1) + [0]
                    csp.add_variable(course_var, domain)

                    # add constraint to limit number of course variables.
                    csp.add_binary_factor((request, quarter), course_var, lambda x, y: \
                        y == 0 if x is None or cid not in x else y in range(course_min_unit, course_max_unit + 1))
                    quarter_course_vars.append(course_var)

            total_unit_var = get_sum_variable(csp, "total_course_unit" + quarter, quarter_course_vars, unit_scope[1])
            csp.add_unary_factor(total_unit_var, lambda total_unit: unit_scope[0] <= total_unit <= unit_scope[1])
        # END_YOUR_CODE
