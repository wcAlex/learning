...
def test2a():
    ...
    print('originalVal={0} ; newVal={1}'.format(originalVal, newVal))
grader.addHiddenPart('2a-hidden', test2a, 3, maxSeconds=60*60, description="Hidden test for CounterexampleMDP. Ensure that V[startState] is greater after noise is added.")
...
def test3a2():
    ...
    mdp3 = submission.BlackjackMDP(cardValues=[5, 10], multiplicity=2,
                                   threshold=15, peekCost=1)
    preEmptyState2 = (16, None, (1, 0))
    ...
    tests = [
        ...
        ([], mdp1, postBustState, 'Quit'),
        ([((12, None, None), 1, 12)], mdp2, preEmptyState, 'Take'),
        ([((11, None, None), 1, 11)], mdp2, preEmptyState, 'Quit'),
        ([((16, None, None), 1, 0)], mdp3, preEmptyState2, 'Quit')
    ]
    for gold, mdp, state, action in tests:
        if not grader.requireIsEqual(gold,
                                     mdp.succAndProbReward(state, action)):
            print '   state: {}, action: {}'.format(state, action)
grader.addBasicPart('3a-basic2', test3a2, 0, description="Basic test for succAndProbReward() that covers several edge cases.")
...
grader.addBasicPart('4a-basic', test4a, 5, maxSeconds=60*60, description="Basic test for incorporateFeedback() using NumberLineMDP.")
...
grader.addBasicPart('4b-helper', run4bHelper, 0, maxSeconds=60*60, description="Helper function to run Q-learning simulations for question 4b.")
