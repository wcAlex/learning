...
class ExactInference(object):
    ...
    def observe(self, agentX, agentY, observedDist):
        ...
        # Update probability for each cell.
        for r in range(self.belief.getNumRows()):
            for c in range(self.belief.getNumCols()):
                x, y = (util.colToX(c), util.rowToY(r))
                dist = math.hypot(agentX - x, agentY - y)
                prev = self.belief.getProb(r, c)
                # Compute p(d_t | c_t), imaging car is at position (x,y).
                # What is probability of of this distance vs this car to another car's distance.
                cur = util.pdf(observedDist, Const.SONAR_STD, dist)
                # Set posterior probability.
                self.belief.setProb(r, c, cur * prev)
    ...
    def elapseTime(self):
        ...
        new_belief = util.Belief(self.belief.getNumRows(), self.belief.getNumCols(), 0.0)
        for pos_pair, prob in self.transProb.items():
            if prob != 0:
                old_pos, new_pos = pos_pair
                prev_posterior = self.belief.getProb(old_pos[0], old_pos[1])
                posterior = prev_posterior * prob
                new_belief.addProb(new_pos[0], new_pos[1], posterior)
...
class ParticleFilter(object):
    ...
    def observe(self, agentX, agentY, observedDist):
        ...
        # Re-weight particles
        weights = {}
        for pos, cnt in self.particles.items():
            weights[pos] = 0
            dist = math.hypot(agentX - util.colToX(pos[1]), agentY - util.rowToY(pos[0]))
            prob = util.pdf(observedDist, Const.SONAR_STD, dist)
            weights[pos] = prob * cnt
    ...
    def elapseTime(self):
        ...
        new_particles = collections.Counter()
        for pos, cnt in self.particles.items():
            weights = self.transProbDict[pos]
            for i in range(cnt):
                new_pos = util.weightedRandomChoice(weights)
